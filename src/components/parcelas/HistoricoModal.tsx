...
export function HistoricoModal({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="w-[95vw] sm:max-w-3xl flex flex-col">
        <DialogHeader>
          <DialogTitle>Histórico da parcela</DialogTitle>
          <DialogDescription>
            {parcela && `Parcela ${parcela.numero_parcela} · ${historico.length} registro${historico.length !== 1 ? 's' : ''}`}
          </DialogDescription>
        </DialogHeader>
        <DialogBody>
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center gap-2">
              <Label htmlFor="filtro-tipo" className="text-xs">Filtrar:</Label>
              <Select value={filtroTipoEvento} onValueChange={setFiltroTipoEvento}>
                <SelectTrigger id="filtro-tipo" className="w-full sm:w-[180px] h-8 text-xs">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            {filteredHistorico.length === 0 ? (
              <p className="text-muted-foreground text-center py-8 text-sm">Nenhum registro encontrado.</p>
            ) : (
              <>
                {/* Desktop */}
                    ...
                    <TableBody>
                      {filteredHistorico.map((item) => (
                        <TableRow key={item.id}>
                          <TableCell className="text-xs">{format(new Date(item.data_pagamento), "dd/MM/yyyy HH:mm", { locale: ptBR })}</TableCell>
                          <TableCell>
                            <Badge variant={item.tipo_evento === "pagamento" ? "default" : item.tipo_evento === "alteracao_data" ? "secondary" : "destructive"}
                              ...
                              {item.tipo_evento === "alteracao_data" && "Data"}
                          ...
                          <TableCell className="max-w-xs truncate text-xs text-muted-foreground">{item.observacao || "—"}</TableCell>
                          <TableCell className="text-right">
                            <Button variant="ghost" size="sm" onClick={() => handleExcluirPagamento(item.id)} className="h-7 w-7 p-0 text-destructive hover:text-destructive">
