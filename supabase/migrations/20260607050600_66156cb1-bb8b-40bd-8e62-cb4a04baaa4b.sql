...
BEGIN
  ...
  FROM (
    SELECT
      ...
      m.mes as mes_order,
      COALESCE(SUM(pg.lucro), 0) as lucro
    ...
    LEFT JOIN (
      SELECT
        p.data_pagamento,
        GREATEST(COALESCE(p.valor_pago, 0) - (c.valor_emprestado / c.numero_parcelas), 0) as lucro
      FROM parcelas p
      JOIN contratos c ON p.contrato_id = c.id
      JOIN clientes cl ON c.cliente_id = cl.id
      WHERE cl.user_id = v_user_id
    ...
    ) pg ON pg.data_pagamento >= m.mes::date
        AND pg.data_pagamento < (m.mes + interval '1 month')::date
    GROUP BY m.mes
  ) t;
