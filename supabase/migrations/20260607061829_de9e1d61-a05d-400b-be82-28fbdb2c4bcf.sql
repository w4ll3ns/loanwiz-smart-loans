...
BEGIN
  ...
  -- Lucro pelo modelo de juros (lido do histórico de pagamentos)
  ...
  -- Lucro mensal (últimos 6 meses) -- modelo de juros + filtro de usuário em subconsulta pré-filtrada
  SELECT COALESCE(jsonb_agg(row_to_json(t)::jsonb ORDER BY t.mes_order), '[]'::jsonb) INTO v_lucro_mensal
  FROM (
    SELECT
      to_char(m.mes, 'Mon/YY') as mes,
      m.mes as mes_order,
      COALESCE(SUM(hh.lucro_evento), 0) as lucro
    ...
    LEFT JOIN (
      SELECT h.data_pagamento::date AS dia,
        CASE
          WHEN h.tipo_pagamento IN ('juros','parcial') THEN COALESCE(h.valor_pago,0)
          WHEN h.tipo_pagamento = 'total' THEN COALESCE(h.valor_pago,0) - (c.valor_emprestado / NULLIF(c.numero_parcelas,0))
          ELSE 0 END AS lucro_evento
      FROM parcelas_historico h
      JOIN parcelas p ON p.id = h.parcela_id
      JOIN contratos c ON p.contrato_id = c.id
      JOIN clientes cl ON c.cliente_id = cl.id
      WHERE cl.user_id = v_user_id AND h.tipo_evento = 'pagamento'
    ) hh ON hh.dia >= m.mes::date AND hh.dia < (m.mes + interval '1 month')::date
    GROUP BY m.mes
  ) t;
