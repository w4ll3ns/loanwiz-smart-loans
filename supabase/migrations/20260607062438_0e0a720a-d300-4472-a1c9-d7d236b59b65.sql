...
BEGIN
  ...
  INTO v_dias, v_recebido_mes, v_previsto_mes, v_atrasado_mes, v_qtd_recebimentos, v_qtd_previstos, v_qtd_atrasados, v_total_emprestado_mes, v_qtd_emprestimos_mes
  FROM computado;
...
DECLARE
  ...
  v_total_emprestado numeric := 0;
  v_qtd_recebimentos integer := 0;
  v_qtd_previstos integer := 0;
  v_qtd_emprestimos integer := 0;
BEGIN
  v_user_id := auth.uid();
  IF v_user_id IS NULL THEN
    RAISE EXCEPTION 'Not authenticated';
  ...
  FROM (
    SELECT
      ...
      p.numero_parcela,
      c.numero_parcelas AS total_parcelas,
      COALESCE(p.valor_original, p.valor)::numeric AS valor_previsto,
  ...
  SELECT
    COALESCE(jsonb_agg(row_to_json(t)::jsonb ORDER BY t.cliente_nome), '[]'::jsonb),
    COALESCE(SUM(t.valor_emprestado), 0),
    COUNT(*)
  INTO v_emprestimos, v_total_emprestado, v_qtd_emprestimos
  FROM (
    SELECT
      c.id AS contrato_id,
      cl.nome AS cliente_nome,
